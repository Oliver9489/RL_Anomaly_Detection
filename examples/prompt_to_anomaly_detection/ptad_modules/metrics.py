...
def calculate_max_f1(gt, scores, flag=False):

    # if np.all(gt == 0) or np.all(scores == 0):
    #     return 0.0, 0.0
    ...
    if flag:
        ...
        print("f1s：", f1s, ",max_f1", max_f1, ",thresholds:", thresholds, ",threshold:", threshold)
    return max_f1, threshold


def safe_divide(a, b, epsilon=1e-7):
    return a / (b + epsilon)
...
def metric_cal(scores, gt_list, gt_mask_list, cal_pro=False):
    ...
    if np.all(gt_mask == 0):
        # in this situation,positive become to find the nomal pixel
        gt_mask = np.logical_not(gt_mask)
        scores = np.logical_not(scores)
    ...
    pxl_f1,pxl_threshold = calculate_max_f1(gt_mask, scores)
    ...
    result_dict = {'p_f1': pxl_f1 * 100,'r_f1': max_f1_region * 100}
